...
EXCEL_PATH = "製品マスタ.xlsx"  # 同じディレクトリに配置すること
...
def webhook():
    ...
    try:
        ...
        return jsonify({"response": f"⚠️ エラーが発生しました：{str(e)}"}), 500
    ...
    # ✅ Dify用に response キーで返却！
    return jsonify({
        "response": f"{item_name} の現在庫数は {stock} 個です。\n→ {status}（発注数：{order}個）\n\n📎 ダウンロードURL：https://dify-stock-order.onrender.com/download/{filename}"
    })
